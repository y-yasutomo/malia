...
#'@examples
#'
 SDAM<-function(Voyage.name,COVARIATE=c("conv","occo","weather","size"),key.list=c("hn","hr","hhn","hhr"),
   ...
   system.time({
     for(i in 1:length(key.list)){
       ...
       for(j in 1:length(COVARIATE)){
         ...
         RES.[[j]]<-TCDM(Voyage.name,type=NULL,key,td,cp=cp,colist=colist,inits=inits,area=area,
